...
// Convert the fixture with extra EasyEDA shape strings appended to the footprint.
const convertWithExtraShapes = (extraShapes: string[]) => {
  const raw = structuredClone(rawJson)
  raw.packageDetail.dataStr.shape.push(...extraShapes)
...
test.failing(
  ...
  () => {
    const baseline = convertWithExtraShapes([])
    const withBottom = convertWithExtraShapes([
