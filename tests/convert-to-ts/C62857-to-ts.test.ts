...
it("converts C62857 capacitor array with its EasyEDA symbol", async () => {
  ...
  expect(result).toContain("<symbol>")
  expect(result.match(/<schematicpath /g)).toHaveLength(25)
  expect(result.match(/<port /g)).toHaveLength(8)
  expect(result).toContain(
    '<port name="pin1" pinNumber={1} aliases={["1"]} direction="left" schX={-0.7} schY={0.46} schStemLength={0} />',
  )
  expect(result).toContain(
    '<port name="pin8" pinNumber={8} aliases={["8"]} direction="right" schX={0.7} schY={0.46} schStemLength={0} />',
  )
  expect(result).toContain(
    '<schematicpath svgPath="M -0.7 0.46 L -0.3 0.46" strokeColor="#000000" />',
  )
  expect(result).toContain(
    '<schematicpath svgPath="M 0.7 0.46 L 0.3 0.46" strokeColor="#000000" />',
  )
